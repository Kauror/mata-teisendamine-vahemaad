...
export async function pingServer(timeoutMs = 5000): Promise<PingResponse | null> {
  if (typeof navigator !== 'undefined' && navigator.onLine === false) return null;
  try {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    const before = Date.now();
    const res = await fetch('/api/offline/ping', { signal: controller.signal, cache: 'no-store' });
    clearTimeout(timer);
