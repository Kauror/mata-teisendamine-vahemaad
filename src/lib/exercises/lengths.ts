...
function arithmetic(d:Difficulty,rng:RNG,i:number):GeneratedQuestion{const add=i%2===0; if(d==='Lihtne'){const u=pick(rng,['cm','dm','m'] as LengthUnit[]); const a=rInt(rng,1,10),b=rInt(rng,1,10); return {id:`a-${i}`,category:'Arvutamine',difficulty:d,question:`Leia ${add?'summa':'vahe'}: ${a} ${u} ${add?'+':'-'} ${Math.min(b,a)} ${u}.`,expectedUnit:u,correctAnswer:add?a+b:a-Math.min(b,a)};} const am=rInt(rng,1,9),ac=rInt(rng,10,90),bm=rInt(rng,1,6),bc=rInt(rng,10,90); const x=am*100+ac,y=bm*100+bc; const hi=Math.max(x,y),lo=Math.min(x,y); return {id:`a-${i}`,category:'Arvutamine',difficulty:d,question:`Leia ${add?'summa':'vahe'}: ${am} m ${ac} cm ${add?'+':'-'} ${bm} m ${bc} cm. Vastus cm-des.`,expectedUnit:'cm',correctAnswer:add?x+y:hi-lo};}

function missing(d:Difficulty,rng:RNG,i:number):GeneratedQuestion{const easy=[['50 cm + ___ cm = 100 cm',50],['5 cm + ___ cm = 10 cm',5],['1 m = ___ cm',100],['___ mm = 8 cm',80]] as const; const med=[['700 m + ___ m = 1 km',300],['2 m 4 dm = ___ cm',240],['___ m = 300 cm',3],['3 dm 5 cm = ___ cm',35]] as const; const arr=d==='Lihtne'?easy:d==='Keskmine'?med:[...med,['1,5 km = ___ m',1500] as const]; const it=arr[i%arr.length]; return {id:`p-${i}`,category:'Puuduv arv',difficulty:d,question:it[0],expectedUnit:'cm',correctAnswer:it[1]};}

function perimeter(d:Difficulty,rng:RNG,i:number):GeneratedQuestion{if(d==='Lihtne'){const s=pick(rng,[2,3,4,5]); return{id:`u-${i}`,category:'Ümbermõõt',difficulty:d,question:`Ruudu külg on ${s} m. Leia ümbermõõt meetrites.`,expectedUnit:'m',correctAnswer:s*4};} const a=rInt(rng,2,d==='Keskmine'?12:35),b=rInt(rng,2,d==='Keskmine'?10:28),c=rInt(rng,2,d==='Keskmine'?10:28); if(i%2===0) return{id:`u-${i}`,category:'Ümbermõõt',difficulty:d,question:`Ristküliku küljed on ${a} m ja ${b} m. Leia ümbermõõt meetrites.`,expectedUnit:'m',correctAnswer:2*(a+b)}; return{id:`u-${i}`,category:'Ümbermõõt',difficulty:d,question:`Kolmnurga küljed on ${a} m, ${b} m ja ${c} m. Leia ümbermõõt meetrites.`,expectedUnit:'m',correctAnswer:a+b+c};}
