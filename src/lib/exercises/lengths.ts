...
function buildJarjestamine(rng:RNG, difficulty:Difficulty, i:number):GeneratedQuestion {
  ...
  type TripletItem = { v: number; u: LengthUnit };
  const triplets: TripletItem[][] = difficulty==='Raske' ? [[{v:2,u:'km'},{v:1500,u:'m'},{v:1800,u:'m'}],[{v:1,u:'m'},{v:95,u:'cm'},{v:12,u:'dm'}]] : [[{v:3,u:'dm'},{v:25,u:'cm'},{v:400,u:'mm'}],[{v:70,u:'cm'},{v:9,u:'dm'},{v:800,u:'mm'}]];
  const t = pick(rng, triplets);
  const vals = t.map((x)=>fromMm(mixedToMm([{value:x.v,unit:x.u}]), difficulty==='Raske'?'m':'cm')).sort((a:number,b:number)=>a-b);
  const ans = target==='lühem'?vals[0]:target==='pikem'?vals[2]:vals[1];
  const outUnit = difficulty==='Raske'?'m':'cm';
  return {id:uid(i),category:'Järjestamine',difficulty,question:`Antud on ${t.map((x)=>`${x.v} ${x.u}`).join(', ')}. Kirjuta kõige ${target} pikkus ${outUnit}-des.`,expectedUnit:outUnit,correctAnswer:ans};
}
