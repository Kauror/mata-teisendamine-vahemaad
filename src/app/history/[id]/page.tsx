...
export default async function HistoryDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className='container'>
      <h1>Tulemus</h1>
      <section className='card'><Link className='chip' href='/?view=kiur-matemaatika'>Tagasi Pikkuste teisendamise juurde</Link>
          ...
          return (
            <article key={q.id}>
              ...
              {q.kind === 'ordering'
                ...
                : <><p>Sinu vastus: {q.userAnswer || '—'} {q.expectedUnit}</p><p>Õige vastus: {q.correctAnswer} {q.expectedUnit}</p></>}
      ...
      <div className='row'>
        ...
        <Link className='btn chip active' href='/?view=kiur-matemaatika'>Vali uus harjutus</Link>
      </div>
    </main>
  );
}
