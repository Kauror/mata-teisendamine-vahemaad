...
export default function Home() {
  ...
  return <main className='container'><h1>Pikkuste harjutaja</h1>
    ...
    {view==='kirsi' && <section className='card'><h2>Kirsi</h2><p>Kirsi tegevused tulevad hiljem.</p><button type="button" onClick={()=>setView('avaleht')}>Tagasi</button></section>}
    {view==='kiur' && <section className='card'><h2>Kiur</h2><button type='button' className='profile' onClick={()=>setView('kiur-matemaatika')}>📘 Matemaatika</button><button type="button" onClick={()=>setView('avaleht')}>Tagasi</button></section>}
    {view==='kiur-matemaatika' && <section className='card'><h2>Pikkusühikud ja teisendamine</h2>
      ...
      <Link className='btn' href={`/test?category=${encodeURIComponent(category)}&difficulty=${difficulty}&count=${count}&seed=${seed}&profile=Kiur`}>Alusta</Link>
      <button type="button" onClick={()=>setView('kiur')}>Tagasi</button></section>}
